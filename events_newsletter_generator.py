...
def generate_article_list(articles: list, output_format: str = "html") -> str:
    """Generate a formatted list of all articles for review with relevance scoring."""
    
    # Keywords that indicate high relevance for global trade/investment focus
    ...
    MEDIUM_PRIORITY_KEYWORDS = [
        "exhibition", "trade show", "conference", "event", "venue",
        "attendee", "exhibitor", "sponsor", "delegate", "visitor",
        "B2B", "media", "publishing", "content"
    ]
    
    def score_article(article):
        """Score article by relevance to key themes."""
        ...
        if article.get('from_user_sources'):
            score += 10  # User sources always prioritized
        
        return score, matched_keywords[:5]  # Return top 5 matched keywords
    ...
    if output_format == "html":
        return _generate_html_article_list(scored_articles)
    else:
        return _generate_text_article_list(scored_articles)


def _generate_html_article_list(articles: list) -> str:
    """Generate HTML article list with clickable links."""
    
    html = """<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>Article Review List</title>
    <style>
        body {
        ...
        h1 { color: #1a1a1a; border-bottom: 3px solid #7B8B70; padding-bottom: 10px; }
        .stats { background: #7B8B70; color: white; padding: 15px; border-radius: 8px; margin-bottom: 20px; }
        ...
        .article.high-relevance { border-left-color: #22c55e; }
        .article.medium-relevance { border-left-color: #f59e0b; }
        .article.low-relevance { border-left-color: #ddd; }
        .article.user-source { border-left-color: #7B8B70; background: #f0f7f0; }
        .article-header { display: flex; justify-content: space-between; align-items: start; }
        .article-number {
            background: #7B8B70;
            color: white;
            padding: 2px 8px;
        ...
        .article-title a:hover { color: #7B8B70; text-decoration: underline; }
        .article-meta { font-size: 12px; color: #666; margin-bottom: 8px; }
        .article-preview { font-size: 14px; color: #444; line-height: 1.5; }
        .keywords {
            margin-top: 8px;
            display: flex;
            flex-wrap: wrap;
...
<body>
    <h1>📋 Article Review List</h1>
    
    <div class="stats">
        <strong>Total Articles:</strong> """ + str(len(articles)) + """ | 
        <strong>High Relevance:</strong> """ + str(len([a for a in articles if a['relevance_score'] >= 9])) + """ |
        <strong>Medium:</strong> """ + str(len([a for a in articles if 3 <= a['relevance_score'] < 9])) + """ |
        <strong>Low:</strong> """ + str(len([a for a in articles if a['relevance_score'] < 3])) + """
    </div>
    
    <div class="legend">
        <h3>🎯 Relevance Scoring</h3>
        <p>Articles are scored and sorted by relevance to: <strong>global trade, foreign investment, capital flows, M&A activity, geographic expansion</strong></p>
        <p>
            <span class="score high">HIGH</span> = Strong match (score 9+) &nbsp;
            <span class="score medium">MEDIUM</span> = Moderate match (3-8) &nbsp;
            <span class="score low">LOW</span> = Weak match (0-2)
        </p>
    </div>
"""
    ...
    for article in articles:
        ...
        if article.get('from_user_sources'):
            relevance_class = "user-source"
        
        user_badge = '<span class="user-badge">YOUR SOURCE</span>' if article.get('from_user_sources') else ''
        
        keywords_html = ""
        if article['matched_keywords']:
            keywords_html = '<div class="keywords">' + ''.join(
                f'<span class="keyword">{kw}</span>' for kw in article['matched_keywords']
            ) + '</div>'
    ...
    <div class="article {relevance_class}">
        <div class="article-header">
            ...
            <span class="score {score_class}">{score_label} ({score})</span>
        </div>
        <div class="article-title">{title_html}{user_badge}</div>
        <div class="article-meta">{article['source']} • {article['published']}</div>
        <div class="article-preview">{article['content'][:300]}...</div>
        {keywords_html}
    </div>
"""
    ...
    <div class="instructions">
        ...
        <p><strong>To customize the newsletter, re-run the workflow with:</strong></p>
        <ul>
            <li><code>include_articles</code>: e.g., "3,7,12" — prioritize these articles</li>
            <li><code>exclude_articles</code>: e.g., "5,9,15" — skip these articles</li>
            <li><code>custom_instructions</code>: e.g., "Focus on Asia-Pacific deals"</li>
        </ul>
        <p><strong>Article numbers shown are the original index (#) — use these numbers in your feedback.</strong></p>
    </div>
</body>
</html>
"""
...
def _generate_text_article_list(articles: list) -> str:
    """Generate plain text article list (fallback)."""
    output = []
    output.append("=" * 70)
    output.append("ARTICLE REVIEW LIST (Sorted by Relevance)")
    ...
    for article in articles:
        ...
        user_flag = " ⭐ YOUR SOURCE" if article.get('from_user_sources') else ""
        
        output.append(f"[#{article['index']}] {relevance}{user_flag}")
        output.append(f"    {article['title']}")
        output.append(f"    Source: {article['source']} | Date: {article['published']}")
        output.append(f"    Link: {article.get('link', 'N/A')}")
        output.append(f"    Preview: {article['content'][:200]}...")
        if article['matched_keywords']:
            output.append(f"    Keywords: {', '.join(article['matched_keywords'])}")
        output.append("")

...
<head>
    ...
    <style>
        body { font-family: Helvetica, Arial, sans-serif; font-size: 10pt; max-width: 900px; margin: 0 auto; padding: 20px; background: #f5f5f5; }
        ...
        .article { background: white; border-radius: 8px; padding: 15px; margin-bottom: 10px; border-left: 4px solid #ddd; }
        ...
        .article-number { background: #7B8B70; color: white; padding: 2px 8px; border-radius: 4px; font-weight: bold; font-size: 9pt; }
        .score { font-size: 9pt; padding: 2px 6px; border-radius: 4px; }
        .score.high { background: #dcfce7; color: #166534; }
        .score.medium { background: #fef3c7; color: #92400e; }
        .score.low { background: #f3f4f6; color: #6b7280; }
        .article-title { font-size: 11pt; font-weight: 600; margin: 8px 0 5px 0; }
        .article-title a { color: #1a1a1a; text-decoration: none; }
        .article-title a:hover { color: #7B8B70; text-decoration: underline; }
        .article-meta { font-size: 9pt; color: #666; }
        .article-preview { font-size: 9pt; color: #444; margin-top: 8px; }
        .keywords { margin-top: 8px; }
        .keyword { background: #e5e7eb; color: #374151; padding: 2px 6px; border-radius: 10px; font-size: 8pt; margin-right: 4px; }
        .instructions { background: #fffbeb; border: 1px solid #f59e0b; padding: 15px; border-radius: 8px; margin-top: 20px; font-size: 9pt; }
    </style>
</head>
<body>
    <h1>📋 Article Review List</h1>
    <div class="stats">
        <strong>Total:</strong> """ + str(len(scored)) + """ | 
        <strong>High Relevance:</strong> """ + str(len([a for a in scored if a['relevance_score'] >= 9])) + """ |
        <strong>Medium:</strong> """ + str(len([a for a in scored if 3 <= a['relevance_score'] < 9])) + """
    </div>
"""
    
    for article in scored:
        score = article['relevance_score']
        rel_class = "high" if score >= 9 else "medium" if score >= 3 else ""
    ...
    <div class="article {rel_class}">
        ...
        <div class="article-preview">{article['content'][:250]}...</div>
...
"""
    ...
    <div class="instructions">
        <strong>To customize:</strong> Re-run workflow with <code>include_articles</code> (e.g., "3,7,12") or <code>exclude_articles</code> (e.g., "5,9")
    </div>
</body>
</html>"""
...
def generate_executive_summary(sections_content: dict, api_key: str) -> str:
    """Generate simple 3-bullet executive summary."""
    ...
    stories_text = "\n".join(all_stories[:10])  # Limit to 10 stories
...
REQUIREMENTS:
1. Start with "Good morning," on its own line
2. Write ONE sentence (max 20 words) setting the context
3. Then write exactly 3 bullet points summarizing the key themes
4. Each bullet should be 1 line, answer "so what?" for executives
5. Use simple, direct language - no jargon
...
SECTIONS:
1. MARKET SIGNALS (sub-themes: "Macro Economy" and "Consumer Trends")
2. DEALS (M&A, investments)
3. HIRES & FIRES (executive moves)

RULES:
- {stories_per_section} stories per section (if enough content)
- Articles marked [MUST INCLUDE] or [PRIORITIZE] get priority
- For Market Signals, tag each story with sub_theme: "Macro Economy" or "Consumer Trends"
